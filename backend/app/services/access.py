...
async def get_student_for_user(session: AsyncSession, user: User) -> Optional[Student]:
...
async def get_tutor_for_user(session: AsyncSession, user: User) -> Optional[Tutor]:
    if role_value(user) != RoleEnum.tutor.value:
        return None
    res = await session.execute(select(Tutor).where(Tutor.user_id == user.id))
    return res.scalar_one_or_none()
