...
# _get_tutor_for_user is now imported from app.services.access
...
async def create_session(
    ...
    *,
    ...
    actor_tutor = await get_tutor_for_user(session, actor)
...
async def _assert_session_access(session: AsyncSession, *, actor: User, ts: TeachingSession) -> None:
    ...
    if v == RoleEnum.tutor.value:
        tutor = await get_tutor_for_user(session, actor)
...
async def approve_session(
    ...
    *,
    ...
    if ts.approval_status not in (SessionApprovalStatusEnum.submitted.value, "pending_review"):
...
async def get_dashboard_data(
    ...
    *,
    ...
    if scoped:
        if discipline and discipline != scoped:
            # We already raised above, but for completeness
            pass
        approved_count_stmt = approved_count_stmt.where(TeachingSession.discipline == scoped)
    elif discipline:
        approved_count_stmt = approved_count_stmt.where(TeachingSession.discipline == discipline)
