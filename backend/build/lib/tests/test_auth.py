...
@pytest.mark.asyncio
async def test_admin_only_super_admin_returns_403_for_student(client: AsyncClient) -> None:
    login_resp = await client.post(
        "/api/v1/auth/login",
        json={"email": SEED_STUDENT_EMAIL, "password": SEED_PASSWORD},
    )
    assert login_resp.status_code == 200
...
@pytest.mark.asyncio
async def test_admin_only_super_admin_allows_super_admin(client: AsyncClient) -> None:
    login_resp = await client.post(
        "/api/v1/auth/login",
        json={"email": SEED_SUPER_ADMIN_EMAIL, "password": SEED_PASSWORD},
    )
