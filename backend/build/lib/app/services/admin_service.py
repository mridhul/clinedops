...
class AdminService:
    ...
    async def get_users(self, skip: int = 0, limit: int = 50, role: str | None = None) -> tuple[Sequence[User], int]:
        query = select(User)
        if role:
    ...
    async def get_audit_logs(
        self, skip: int = 0, limit: int = 50, actor_id: UUID | None = None, action: str | None = None
